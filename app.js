...
function clearAll() {
  ...
  state.origPalette = [];
  state.restricted = [];
  state.mixRule = { srcHex:null, inks:[], block:6, pattern:'blue' };
...
els.btnEyedrop.addEventListener('click', () => { if(!state.srcW) return toast('Load an image first','danger'); state.eyedropActive=true; status('Eyedropper active — click the source image'); });
els.srcCanvas.addEventListener('click', (e) => {
  if (!state.eyedropActive) return; state.eyedropActive=false;
...
function samplePixels(ctx, max = 120000) {
  ...
  const step=Math.max(1, Math.floor((data.length/4)/max));
  const out=[];
...
function autoExtract(){
  ...
  if(!samples.length) return toast('No pixels to sample','danger');
...
if(els.mixPattern) els.mixPattern.addEventListener('change', ()=>{ state.mixRule.pattern = els.mixPattern.value || 'blue'; });
...
els.btnPreviewMix.addEventListener('click',()=>{
  ...
  const tile=buildDotTile(state.mixRule.block, inks, state.mixRule.pattern);
...
// NEW: Interleaved (round-robin) tiler with patterns
function buildDotTile(b, inks, pattern = 'blue') {
  ...
  if (pattern === 'checker') {
    positions.sort((p, q) => ((p.x + p.y) & 1) - ((q.x + q.y) & 1));
  } else if (pattern === 'stripes-h') {
    positions.sort((p, q) => (p.y - q.y) || (p.x - q.x));
  } else if (pattern === 'stripes-v') {
    positions.sort((p, q) => (p.x - q.x) || (p.y - q.y));
  } else if (pattern === 'bayer') {
    const M = [[0,8,2,10],[12,4,14,6],[3,11,1,9],[15,7,13,5]];
    positions.sort((p,q) => (M[p.y % 4][p.x % 4]) - (M[q.y % 4][q.x % 4]));
  } else {
  ...
  for (const pos of positions) {
    ...
    const idx = (pos.y * tile.width + pos.x) * 4;
...
/* -------------------- Stage 5: Mapping (Worker + Preview Scale) -------------------- */
...
function getScaledSrcImageData(scale){ const sw=state.srcW, sh=state.srcH; const w=Math.max(1,Math.round(sw*scale)), h=Math.max(1,Math.round(sh*scale)); const off=document.createElement('canvas'); off.width=w; off.height=h; const cx=off.getContext('2d',{willReadFrequently:true}); cx.imageSmoothingEnabled=true; cx.drawImage(els.srcCanvas,0,0,w,h); return { imageData: cx.getImageData(0,0,w,h), width:w, height:h }; }
...
function getMapperWorker(){
  ...
  // Interleaved tiler with pattern ordering (matches main-thread version)
  function buildDotTile(b, inks, pattern){
    const tile = new ImageData(b,b);
    const cells=b*b;
  ...
  onmessage = (e) => {
    ...
    let mixTile=null, block=6, hasMix=false;
    if(mixRule && mixRule.srcHex && mixRule.inks && mixRule.inks.length>=2){
      block = Math.max(2, Math.min(64, mixRule.block||6));
      mixTile = buildDotTile(block, mixRule.inks, mixRule.pattern||'blue');
      hasMix = true;
    }

    const err = dither ? new Float32Array((width*height)*3) : null;
    const origRGB = (mixRule && mixRule.origPalette)||[];
    ...
    for(let y=0;y<height;y++){
      for(let x=0;x<width;x++){
        ...
        if(dither){
          ...
          r=clamp(r + err[i3]); g=clamp(g + err[i3+1]); b=clamp(b + err[i3+2]);
        }
    ...
    if(sharpen){
      ...
      for(let i=0;i<out.length;i+=4){
        out[i]=clamp(out[i] + amt*(out[i]-tmp[i]));
        out[i+1]=clamp(out[i+1] + amt*(out[i+1]-tmp[i+1]));
        out[i+2]=clamp(out[i+2] + amt*(out[i+2]-tmp[i+2]));
      }
    }
...
function currentParamsHash(){
  const mix = (state.mixRule && state.mixRule.inks && state.mixRule.inks.length>=2)
    ? { srcHex: state.mixRule.srcHex, inks: state.mixRule.inks, block: state.mixRule.block, pattern: state.mixRule.pattern||'blue', origPalette: state.origPalette.slice() }
    : null;
...
function mapToPalette(){
  ...
  const previewScale=parseFloat(els.previewRes?.value||'1');
  ...
  els.mapProgress.hidden=false;
  els.mapProgressLabel.textContent='Processing…';
  status(`Mapping preview at ${Math.round(previewScale*100)}%…`);
  ...
  wkr.onmessage=(e)=>{
    ...
    if(type==='done'){
      ...
      lastMapSettings.previewScale=previewScale;
      lastMapSettings.paramsHash=paramsHash;
      lastMapSettings.isFullRes=(previewScale===1);
...
/* -------------------- Stage 6: Export (always full-res remap) -------------------- */
...
listProjects();
els.btnSaveProject.addEventListener('click',()=>{ if(!state.srcW) return toast('Load an image first','danger'); const name=prompt('Project name?')||'Untitled'; const srcDataURL=els.srcCanvas.toDataURL('image/png'); const proj={ name, ts:Date.now(), srcDataURL, origPalette:state.origPalette, restricted:state.restricted, allowWhite:state.allowWhite, mixRule:state.mixRule, settings:{ wL:+els.wL.value, wC:+els.wC.value, dither:!!els.dither.checked, sharpen:!!els.sharpen.checked } }; const arr=listProjects(); arr.unshift(proj); saveProjects(arr); listProjects(); toast('Project saved'); });
els.btnLoadProject.addEventListener('click',()=>{ const arr=listProjects(); const idx=+els.projectSelect.value; if(Number.isNaN(idx)) return; const p=arr[idx]; if(!p) return; const img=new Image(); img.onload=()=>{ drawToSrc(img); state.origPalette=p.origPalette||[]; state.restricted=p.restricted||[]; state.allowWhite=!!p.allowWhite; els.allowWhite.checked=state.allowWhite; state.mixRule={ srcHex:null, inks:[], block:6, pattern:'blue', ...p.mixRule }; els.wL.value=p.settings?.wL ?? 1.0; els.wC.value=p.settings?.wC ?? 1.0; els.dither.checked=!!(p.settings?.dither); els.sharpen.checked=!!(p.settings?.sharpen); if(els.mixPattern) els.mixPattern.value=state.mixRule.pattern||'blue'; renderOrigPalette(); renderRestricted(); renderReplaceSrc(); toast('Project loaded — click “Apply mapping” to render'); }; img.src=p.srcDataURL; });
els.btnDeleteProject.addEventListener('click',()=>{ const arr=listProjects(); const idx=+els.projectSelect.value; if(Number.isNaN(idx)) return; if(!confirm('Delete selected project?')) return; arr.splice(idx,1); saveProjects(arr); listProjects(); toast('Project deleted'); });
